...
const getItemStyles = (id, activeId) => (id === activeId ? s.activeItem : s.item);

const AuthorListView = ({ authors = [], selectAuthor, activeAuthorId }) =>
  authors.length > 0 ? (
    <ul className={s.list}>
      {authors.map(author => (
        <li key={author.id} className={getItemStyles(author.id, activeAuthorId)}>
